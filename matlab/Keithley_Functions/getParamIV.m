function [File,quitProgram] = getParamIV(File)
%% Constants
% Dialog
% buttons
...
while confirmVoltageParam == false
    ...
    promptInputVoltageStart = {...                      % input prompt
        ...
        'Step Size (V) {\bf(Use negative to move left first)}:',...% step size (V)
    ...
    voltageStep = str2double(stepSize_cell);           % step size (double)
