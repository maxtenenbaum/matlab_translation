...
%         fprintf('%d to %d, out of %d\n',idx,idx_final,data_len);
        currentInterval = current(interval);
        currentInterval_len = length(currentInterval);
        if currentInterval_len > 60
            shift = 5;
        else
            shift = 1;
        end
...
%         plot(interval,currentInterval,'LineWidth',1);
        if all(currentInterval > NANOAMP)
            currentStart = mean(currentInterval(1:shift));
            currentLast = mean(currentInterval(end-shift-1:end));
            isCurrentIntervalChange = currentLast > currentStart;
%             coeff = polyfit(interval,currentInterval,1);
%             slope = coeff(1);
            [slope,~,~] = getLinReg(interval,currentInterval);
